  ...
  return new Promise(function (resolve, reject) {
    try {
      ...
      query.on("error", function (err) {
        resolve(
          produceResponseObjectForText(
            "Error listing pins, go hassle Mike. " + err
          )
        );
      });
    } catch (e) {
...
function showPin(pinName) {
  if (pinName.length > 20) {
    return produceImmediateResponse(
      "Pin name must be shorter than 20 characters"
    );
  }
  ...
  return new Promise(function (resolve, reject) {
    try {
      ...
      var query = client.query(
        new pg.Query("SELECT * FROM pins WHERE name=$1", [pinName])
      );
      query.on("row", function (row, result) {
        result.addRow(row);
      });
      query.on("end", function (result) {
        if (result.rowCount === 0) {
          resolve(
            produceResponseObjectForText(
              'The pin "' + pinName + '" is not registered.'
            )
          );
          return;
          ...
          resolve(
            produceResponseObjectForText(
              "Some shit has gone seriously awry. Take cover."
            )
          );
          return;
        }
        ...
        resolve(
          produceResponseObjectForText(pinName + ": " + result.rows[0].content)
        );
      });
      query.on("error", function (err) {
        resolve(
          produceResponseObjectForText(
            "Error showing pin content, go hassle Mike. " + err
          )
        );
      });
    } catch (e) {
...
function createPin(pinName, pinContent) {
  ...
  if (pinContent.length > 300) {
    return produceImmediateResponse(
      "Pin content must be shorter than 300 characters"
    );
  }
      ...
      selectQuery.on("end", function (result) {
        if (result.rowCount === 0) {
          ...
          query.on("error", function (err) {
            resolve(
              produceResponseObjectForText(
                "Error creating pin, go hassle Mike. " + err
              )
            );
          });
        } else {
