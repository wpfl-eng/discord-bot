...
**Date:** 2026-08-31
**Status:** Implemented on `feat/ask`; revised after the Stage 14 review (2026-09-02); awaiting AJ's handoff steps (§17.6)
**Author:** Claude + AJ
...
>
> **Third revision, 2026-09-02.** A second adversarial review, run as an
> interview with AJ (log Stage 14), changed nineteen decisions and made the
> branch's first live `query()` calls. The sections below are rewritten to be
> true rather than annotated with corrections; the log's Stage 14 entry
> records what each one was before and why it changed.
...
| MCP schemas | All eight loaded upfront (`alwaysLoad` on the server); tool search deferral declined under ten tools |
...
| Path confinement | `Read(//DATA_DIR/**)` allow rule **plus** a `PreToolUse` realpath hook that also checks Glob's pattern |
| Setting sources | `[]` (never load the repo's `.claude/` config) |
| `cwd` | The data dir, never the repo |
| Surface | `/ask` plus `/ask-admin`; public thread where possible, in-place where not; the opener continues by typing, others by addressing the bot; soft cap 15, hard 20; one run at a time per thread |
| Progress UX | Ticker of tool calls settled on their results, + reasoning summaries, then streamed prose on ~1.5 s edits; 👍/👎 under the answer |
...
| Limits | 20/day/user, 1500 queries/month league-wide, counting only runs that reached the model; $1.00 `maxBudgetUsd` per query |
...
| ESPN client | The fork at `b8c2e61` (`2.1.0-wpfl.0`), with generated types; positions read from `eligiblePositions` |
...
**The decade cache has a window of its own** (`WPFL_CACHE_STALE_AFTER_MS`, 24 h).
An unchanged etag short-circuits only while the cache is also fresh; otherwise
the full path runs — a second's download and a five-millisecond shred against
the thirteen fetches that are the point. The cache used to refresh only when
the artifact's etag changed, which in season meant only when draft-2026 was
republished. `ensureFresh({ force: true })`, reached through `/ask-admin
resync`, skips both windows and the etag for the Tuesday after a republish.
...
**`seasonMax` is the current NFL season, not a hardcoded 2025.** The table above
was measured while 2026 was empty. The API is loaded in season — commit
`c95de89` bumped `/ewins` and `/optimal` to 2025 on 2025-11-04, mid-season — so
the cache refreshes on its own daily window (§3.5) and `INDEX.md` states where
...
allowedTools: [                   // permission
  `Read(//${DATA_DIR_WITHOUT_LEADING_SLASH}/**)`,   // the documented form: //home/…
...
- **The `//` prefix means an absolute filesystem path.** A single leading slash
  ...
  difference. The documented form is `//home/…`, two slashes then the path
  without its own leading slash; `DATA_DIR` sheds its first slash when the rule
  is built. (It rendered with three slashes until Stage 14; the CLI's parser
  drops one character after `//` and normalises, so it worked by accident.)
  `DATA_DIR` itself is resolved absolute with a leading `~` expanded, because
  dotenv expands nothing and `.env.sample` describes the override as
  `$HOME/wpfl-data`.
...
**Tool search is on by default and defers SDK MCP schemas**, and loading a
deferred one costs a model round trip on a ticker somebody is watching. The
SDK's own guidance is to load everything upfront under ten tools, and the eight
serialise to about two thousand tokens, cached after the first turn. So the
server is declared `alwaysLoad: true` and every schema rides in the initial
prompt. (Stage 14 reversed an earlier three-loaded, five-deferred split that
had been drawn against an empty preseason, when boxscores and free agents
looked rare. Verified live: the init message lists all eight, and
`espn_free_agents` is called with no search turn ahead of it.)
...
**The fork, as it stands.** `package.json` pins
`git+https://github.com/aboorde/ESPN-Fantasy-Football-API.git#b8c2e61…`,
version `2.1.0-wpfl.0`, which ships its own generated TypeScript declarations;
main repinned there on 2026-09-01 and deleted the hand-written types. The
history of how the pin got here — upstream 2.0.1 lacked `getRecentActivity`, the
first fork build had four retired hosts — is in the log's Stage 2. Two facts
about this build matter to the tools:

- **It returns real team names and an `ownerName`.** Verified live 2026-09-02
  (`Ball's Balls` / `Nixon Ball`). Owners still route through
  `constants/wpflMembers.ts`, because ESPN's spellings are whatever an owner
  typed and drift, not because it is the only mapping available.
- **Its `defaultPosition` is a slot label, not a position.** The field is the
  slot whose id equals the player's position id, so the free-agent pool arrives
  as `TQB, RB, RB/WR, WR, WR/TE, D/ST`: a real WR labelled `RB/WR`, a real TE
  labelled `WR`, a kicker `WR/TE`. The first live question asked for wide
  receivers and was handed four tight ends. Both tools now read the bare
  position from `eligiblePositions`, in a fixed order, with the label as the
  fallback. The fix belongs in the fork.

**One constraint that stands:** `espn_transactions` is current-season only.
ESPN serves `/communication` for the current season and 404s for prior ones
(2024 → 404, 2025 → 404, 2026 → 200), independent of client. The tool
description says so, or the agent burns turns retrying a historical question.

**The week comes from ESPN.** Every ESPN tool defaults its week through
`helpers/espnPeriod.ts`, which reads `currentScoringPeriodId` with a fifteen-
minute cache and a calendar fallback — the same helper `/median` and
`/closestscores` use, so the tools' default, the week the prompt states and
the week those commands print are one number. `espn_boxscores` passes matchup
and scoring periods separately.
...
const options: Options = {
  ...
  allowedTools: [...],            // §4.1 — Read(//home/…) in the documented form
...
| Message | Action |
| --- | --- |
| `stream_event` → `content_block_start` with `tool_use` | Push a ticker line, keyed by the `tool_use` id |
| `stream_event` → `content_block_start` with `thinking` | Reset the thinking buffer |
| `stream_event` → `content_block_delta` / `input_json_delta` | Accumulate tool input for the ticker's detail text |
| `stream_event` → `content_block_delta` / `thinking_delta` | Append to the thinking buffer; the ticker shows the whole block (summaries arrive as fragments — a member saw "I") |
| `stream_event` → `content_block_delta` / `text_delta` | Append to the prose buffer, the live preview |
| `user` with `tool_result` blocks | Settle the ticker line whose id matches; ✗ with the first line of an error. Not `assistant`: the SDK emits one per content block, so the one carrying the call lands before the tool runs |
| `assistant` with `error` in the six ops-failure codes | Mark the run uncounted and carry the code to the answer |
| `result` | Record spend, turns, `modelUsage`; on success the published text is its `result`, which is the final turn alone — the stream also holds whatever the model said before its first tool call |
...
**What a member is charged for.** A ledger row is written on every terminal
...
- A **per-thread queue** ahead of everything: one run at a time per thread,
  or two resumes of one SDK session race and one is forgotten. Two may wait
  behind the one in flight; past that a message is refused with a reply and
  costs no cap slot.
...
stretches, which is exactly the stretch the ticker exists to cover. A ✓ means
the tool's result arrived, matched by id; a ✗ means it came back as an error,
with the first line of why, which is how a hook denial becomes legible.
...
**Mentions.** Every send and edit carrying text somebody else wrote — the
question, the model's tool inputs, its prose — parses no mentions, as the
casino renderers already do. Refusal replies keep the default so the person
refused is pinged.

**Feedback.** Two buttons under the last part of every answer, 👍 and 👎, with
counts that update in place on each click; one vote per person per message,
keyed by the message so a vote survives a failed ledger write. A thumbs-down
also logs a line with the thread id. Triage, not learning.
...
that was already verified and cannot touch the snowflakes, and it would need an
alias list to avoid warning on every boot. Recorded as a follow-up.

---

## 8. Data Model

New migration `migrations/014_ask_agent.sql` — numbered 014 because a 009
already existed on main when this branch's file was written; it was renamed
before ever being applied. Four tables. Note the repo has **no automated
migration runner**; migrations are applied by hand (`scripts/runMigration.ts`
exists for this). Beyond the DDL below, Stage 14 added `ask_usage.counted`,
`ask_usage.error` and `ask_usage.message_id`, `ask_sessions.bot_thread`, and an
`ask_feedback` table (one row per person per answer, unique on message and
user, rating constrained to ±1). Applied twice on Postgres 16 in a container.
...
export const ASK = {
  ...
  THREAD_QUEUE_DEPTH: 2,             // waiting behind the one in flight, per thread
...
the hook resolves every path-like argument with `realpath` (so symlinks cannot
escape) and denies anything that does not sit under `ASK.DATA_DIR`. Glob's
`pattern` is a path too: measured live in Stage 14, with no `path` the CLI
...
| --- | --- | --- |
| `@anthropic-ai/claude-agent-sdk` | add, pinned exactly `0.3.252` | The agent runtime; this branch measured CLI-specific behaviour, so an upgrade is a deliberate act with a re-run smoke test |
| `@duckdb/node-api` | add, pinned exactly `1.5.5-r.4` | Read-only SQL over the shred + cache |
| `@modelcontextprotocol/sdk` | add, `^1.29.0` | `CallToolResult`, which the agent SDK uses but does not re-export |
...
### 15.2 The ESPN fork — RESOLVED 2026-08-31, revised 2026-09-02
Fixed at `591ee59`, then repinned by main to `b8c2e61` (`2.1.0-wpfl.0`) with
generated types, real team names and `ownerName`. One residual: its
`defaultPosition` is a slot label, worked around in the tools by reading
`eligiblePositions` (§4.2); the fix belongs in the fork.
...
### 15.5 Path confinement — MEASURED 2026-09-02
With a credential on the box, the question was put to the model: Glob with
pattern `/etc/host*` and no `path`. Seven `/etc` paths came back. The hook
read only `path` and `file_path`, and the CLI honours an absolute pattern. So
the allow rule was not redundant protection and the hook had a hole; the hook
now checks Glob's pattern too (§10.2), and the probe is recorded in the log's
Stage 14 as the first live measurement on the branch. Names, not contents —
a Read of anything it found was still denied — but an agent that could list
the host was not what §10.2 promised.
...
### 15.11 Per-question cost — MEASURED 2026-09-02
The SDK's local estimate, on the branch's first live questions: $0.13 for a
free-agent lookup (2 turns, 8 s), $0.18 for a nine-query ten-year SQL question
(10 turns, 23 s), $0.01 for a follow-up answered from context (1 turn, 3 s).
Inside the design's arithmetic for single questions; the SQL question ran
higher. On the Max token it is a consumption proxy. `ask_usage` records it
from here.

### 15.12 A Claude credential — PROVIDED 2026-09-02
AJ put a `CLAUDE_CODE_OAUTH_TOKEN` in `.env` on the dev box for Stage 14. The
Agent SDK integration has now executed against a real model: auth, permission
configuration, hooks, all eight MCP tools, resume, and the ticker's stream
handling. The Discord layer has not; that needs the migration applied and the
commands deployed, which are AJ's (§17.5).
